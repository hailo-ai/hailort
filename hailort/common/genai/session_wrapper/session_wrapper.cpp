...
Expected<uint64_t> SessionWrapper::get_file_size(const std::string &file_path)
{
...
hailo_status SessionWrapper::send_file_chunked(const std::string &file_path, std::chrono::milliseconds timeout)
{
    std::unique_lock<std::mutex> lock(m_mutex);
    TimeoutGuard timeout_guard(timeout);
    ...
    uint64_t total_file_size = static_cast<uint64_t>(file.tellg());
    file.seekg(0, std::ios::beg);

    // Calculate number of chunks
    size_t chunk_size = CHUNKED_TRANSFER_CHUNK_SIZE;
    uint64_t total_chunks = (total_file_size + chunk_size - 1) / chunk_size;
    ...
    for (uint64_t chunk_index = 0; chunk_index < total_chunks; ++chunk_index) {
        ...
        uint64_t remaining_bytes = total_file_size - chunk_offset;
