...
void Yolov5SegPostProcess::mult_mask_vector_and_proto_matrix(const DetectionBbox &detection)
{
    ...
    // We crop the proto layer to the detected bbox size
    // We do it by copying the relevant part (the bbox part) of the proto layer to the crop buffer
    // We do it because it reduces the matrix multiplication complexity: instead of 
    // multiplying the full proto layer, we multiply the cropped proto layer
    auto x_min = detection.get_bbox_x_min(shape.width);
    auto y_min = detection.get_bbox_y_min(shape.height);
    auto box_width = detection.get_bbox_width(shape.width);
    auto box_height = detection.get_bbox_height(shape.height);

    auto mask_size = static_cast<uint32_t>(detection.get_mask_size_in_bytes(static_cast<uint32_t>(shape.height),
        static_cast<uint32_t>(shape.width)));
    ...
    Eigen::Map<Eigen::Matrix<float, MASK_COEFFICIENT_SIZE, Eigen::Dynamic, Eigen::RowMajor>> proto_layer(
        crop_buffer, MASK_COEFFICIENT_SIZE, mask_size);
    ...
    Eigen::Map<Eigen::Matrix<float, VECTOR_DIM, Eigen::Dynamic, Eigen::RowMajor>> result(
            (float32_t*)m_mask_mult_result_buffer.data(), VECTOR_DIM, mask_size);

    // We don't perform the sigmoid on the mask at this point - we perform inverse-sigmoid on the mask threshold instead.
...
hailo_status Yolov5SegPostProcess::crop_and_copy_mask(const DetectionBbox &detection, MemoryView &buffer, uint32_t buffer_offset)
{
    ...
    uint32_t proto_width = detection.get_bbox_width(proto_layer_shape.width);
    uint32_t proto_height = detection.get_bbox_height(proto_layer_shape.height);
    uint32_t bbox_width = detection.get_bbox_width(static_cast<uint32_t>(yolov5_config.image_width));
    uint32_t bbox_height = detection.get_bbox_height(static_cast<uint32_t>(yolov5_config.image_height));
    stbir_resize_float_generic((float32_t*)m_mask_mult_result_buffer.data(), proto_width,
        proto_height, 0, resized_mask_to_image_dim_ptr, bbox_width,
        bbox_height, 0, 1, STBIR_ALPHA_CHANNEL_NONE, 0,
        STBIR_EDGE_CLAMP, STBIR_FILTER_TRIANGLE, STBIR_COLORSPACE_LINEAR, NULL);

    // write result
    uint8_t *dst_mask = (uint8_t*)(buffer.data() + buffer_offset);
    for (uint32_t i = 0; i < bbox_height; i++) {
        for (uint32_t j = 0; j < bbox_width; j++) {
            auto cropped_mask_idx = (i * bbox_width) + j;
