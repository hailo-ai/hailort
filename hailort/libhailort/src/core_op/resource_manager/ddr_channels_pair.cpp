...
Expected<DdrChannelsPair> DdrChannelsPair::create(HailoRTDriver &driver, const DdrChannelsInfo &ddr_channels_info)
{
    auto buffer_exp = should_use_ccb(driver) ?
        ...
        create_sg_buffer(driver, ddr_channels_info.row_size, ddr_channels_info.min_buffered_rows, ddr_channels_info.d2h_channel_id);
    ...
    auto desc_count_local = buffer_ptr->program_descriptors(total_size, interrupts_domain, 0, true);
...
Expected<std::unique_ptr<vdma::VdmaBuffer>> DdrChannelsPair::create_sg_buffer(HailoRTDriver &driver,
    uint32_t row_size, uint16_t buffered_rows, vdma::ChannelId d2h_channel_id)
{
    ...
    // DdrChannels are circular so we need to allocate the full descriptors list.
...
bool DdrChannelsPair::should_use_ccb(HailoRTDriver &driver)
{
    ...
    case HailoRTDriver::DmaType::DRAM:
        if (std::getenv("HAILO_FORCE_DDR_CHANNEL_OVER_DESC") != nullptr) {
