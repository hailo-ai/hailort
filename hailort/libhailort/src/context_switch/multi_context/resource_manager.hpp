...
public:
    ...
    Expected<std::shared_ptr<VdmaChannel>> create_boundary_vdma_channel(const vdma::ChannelId &channel_id,
        uint32_t transfer_size, const std::string &network_name, const std::string &stream_name,
    ...
    Expected<std::reference_wrapper<context_info_t>> add_new_context(CONTROL_PROTOCOL__context_switch_context_type_t type)
    {
        CHECK_AS_EXPECTED(type < CONTROL_PROTOCOL__CONTEXT_SWITCH_CONTEXT_TYPE_COUNT, HAILO_INVALID_ARGUMENT);
        ...
        auto &context_info = m_contexts.back();
        context_info.context_type = static_cast<uint8_t>(type);
        m_total_context_count++;
        if (CONTROL_PROTOCOL__CONTEXT_SWITCH_CONTEXT_TYPE_DYNAMIC == type) {
            m_dynamic_context_count++;
        }
        
        return std::ref(context_info);
    }
    ...
    hailo_status flush_boundary_input_channels();
...
private:
    ...
    static Expected<uint8_t> get_cfg_channel_engine(HailoRTDriver &driver, uint8_t config_stream_index,
    ...
    ResourcesManager(VdmaDevice &vdma_device, HailoRTDriver &driver,
        ...
        std::vector<ConfigBuffer> &&preliminary_config, std::vector<std::vector<ConfigBuffer>> &&dynamic_config,
