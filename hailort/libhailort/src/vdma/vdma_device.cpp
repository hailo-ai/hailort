...
{
    return m_driver->fw_control(request_buffer, request_size, response_buffer, response_size, cpu_id);
}
...
Expected<size_t> VdmaDevice::read_log(MemoryView &buffer, hailo_cpu_id_t cpu_id, bool should_clear)
{
    size_t read_bytes = 0;
    hailo_status status = HAILO_UNINITIALIZED;
    status = m_driver->read_log(buffer.data(), buffer.size(), &read_bytes, cpu_id, should_clear);
    CHECK_SUCCESS_AS_EXPECTED(status);
    return read_bytes;
}
