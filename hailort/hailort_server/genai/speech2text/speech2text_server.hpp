...
{
public:
    static Expected<std::unique_ptr<Speech2TextServer>> create_unique(std::shared_ptr<Session> session);

    Speech2TextServer(std::shared_ptr<Session> session);
    ...
    virtual ~Speech2TextServer() = default;
...
private:
    ...
    Eigen::Map<Eigen::Matrix<float32_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> reorder_outputs(MemoryView output_buffer);
    ...
    std::unique_ptr<DecoderInferenceManager> m_inference_manager_decoder;
    ...
    Buffer m_decoder_reordered_output_buffer;   // TODO: HRT-18595 - Remove
    std::vector<size_t> m_embeddings_outputs_cols_sizes;
    size_t m_vocab_size;
...
{
public:
    static Expected<std::unique_ptr<Speech2TextServerManager>> create(std::shared_ptr<Session> session);
