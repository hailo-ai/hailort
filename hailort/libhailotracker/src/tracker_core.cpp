    ...
    for (auto &tracklet : m_tracklets) {
        ...
        if ((HAILO_TRACKLET_STATE_TRACKED == tracklet.state()) || 
            ...
            if (m_config.enable_kalman_filter) {
                ...
                tracklet.update_smoothed_from_kalman(m_config.smoothing_alpha);
            }
        }
    }
}
...
void Tracker::update(const hailo_detections_t *detections)
{
    ...
    // Perform association
    m_association.run(detections, m_tracklets);
