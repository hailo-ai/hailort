...
hailo_status UsbEventLoop::read_request(uint8_t request_code_raw, size_t request_length, Buffer &event_buffer)
{
    ...
    case UsbControlProtocolOpcode::CONNECT:
    {
        ...
        auto expected_interface = open_interface();
        if (!expected_interface) {
            m_response.connect_response.status = static_cast<uint32_t>(expected_interface.status());
            break;
        }

        auto interface = expected_interface.release();
        ...
        m_response.connect_response.interface = interface;
    ...
    case UsbControlProtocolOpcode::CLOSE:
    {
        ...
        auto status = close_interface(request->interface);
        if (HAILO_SUCCESS != status) {
            m_response.close_response.status = static_cast<uint32_t>(status);
            break;
        }

        status = handler->handle_close(request->interface);
        if (HAILO_SUCCESS != status) {
            m_response.close_response.status = static_cast<uint32_t>(status);
            break;
        }
...
Expected<usb_interface_t> UsbEventLoop::open_interface()
{
    CHECK(!m_available_interfaces.empty(), HAILO_DEVICE_TEMPORARILY_UNAVAILABLE, "No available interfaces");
    auto interface = *m_available_interfaces.begin();
    m_available_interfaces.erase(interface);
    return interface;
}

hailo_status UsbEventLoop::close_interface(usb_interface_t interface)
{
...
hailo_status UsbEventLoop::drop_pending_request()
{
    if (UsbControlProtocolOpcode::CONNECT == m_pending_opcode) {
        auto status = close_interface(m_response.connect_response.interface);
        CHECK_SUCCESS(status);
    }
...
hailo_status UsbEventLoop::write_response(uint8_t request_code_raw, size_t request_length)
{
    ...
    CHECK(m_pending_opcode == request_code, HAILO_INVALID_OPERATION,
        "Received response-request for an different opration than the pending one, pending opcode: {}, requested opcode: {}",
...
hailo_status UsbEventLoop::loop()
{
    ...
    while (::read(*m_control_ep_fd, &event, sizeof(event)) > 0) {
        ...
        default:
            LOGGER__INFO("Got unhandled USB event: {}", event.type);
            break;
        }
    }
